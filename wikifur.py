...
from exceptions import (
  ...
  wikifurException, ODD_ERROR_MESSAGE)
...
def set_lang(prefix):
  ...
  Set `prefix` to one of the two letter prefixes found on the `list of all wikifurs <http://meta.wikimedia.org/wiki/List_of_wikifurs>`_.
  After setting the language, the cache for ``search``, ``suggest``, and ``summary`` will be cleared.
  .. note:: Make sure you search for page titles in the language that you have set.
  '''
  global API_URL
  API_URL = 'http://' + prefix.lower() + 'wikifur.com/w/api.php'
...
def set_rate_limiting(rate_limit, min_wait=timedelta(milliseconds=50)):
  ...
  load on wikifur, the number of requests you and other `wikifur` users
  are making, and other factors), wikifur may return an HTTP timeout error.
...
def geosearch(latitude, longitude, title=None, results=10, radius=1000):
  '''
  Do a wikifur geo search for `latitude` and `longitude`
...
def suggest(query):
  '''
  Get a wikifur search suggestion for `query`.
...
def random(pages=1):
  '''
  Get a list of random wikifur article titles.
  .. note:: Random only gets articles from namespace 0, meaning no Category, User talk, or other meta-wikifur pages.
  Keyword arguments:
  * pages - the number of random pages returned (max of 10)
  '''
  #http://en.wikifur.org/w/api.php?action=query&list=random&rnlimit=5000&format=jsonfm
  query_params = {
    'list': 'random',
    'rnnamespace': 0,
    'rnlimit': pages,
...
def summary(title, sentences=0, chars=0, auto_suggest=True, redirect=True):
  ...
  * auto_suggest - let wikifur find a valid page title for the query
...
def page(title=None, pageid=None, auto_suggest=True, redirect=True, preload=False):
  '''
  Get a wikifurPage object for the page with title `title` or the pageid
...
class wikifurPage(object):
  '''
  Contains data from a wikifur page.
  ...
  def __init__(self, title=None, pageid=None, redirect=True, preload=False, original_title=''):
    ...
    if preload:
      for prop in ('content', 'summary', 'images', 'references', 'links', 'sections'):
        getattr(self, prop)
  def __repr__(self):
    return stdout_encode(u'<wikifurPage \'{}\'>'.format(self.title))
  ...
  def __load(self, redirect=True, preload=False):
    '''
    Load basic information from wikifur.
  ...
  def revision_id(self):
    ...
    <http://en.wikifur.org/wiki/wikifur:Revision>`_ for more
    information.
    '''
  ...
  def links(self):
    '''
    List of titles of wikifur page links on a page.
    .. note:: Only includes articles from namespace 0, meaning no Category, User talk, or other meta-wikifur pages.
    '''
...
def languages():
  ...
  Can be inputted to `set_lang` to change the Mediawiki that `wikifur` requests
  results from.
  Returns: dict of <prefix>: <local_lang_name> pairs. To get just a list of prefixes,
  use `wikifur.languages().keys()`.
  '''
...
def _wiki_request(params):
  '''
  Make a request to the wikifur API using the given search parameters.
